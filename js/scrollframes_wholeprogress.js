...
function FrameScroller (options, sequences, jPlayerObject) {
  ...
  // Extra function for progress per whole project version
  ...
  function calcTotalFrames () {
    ...
    console.log("calculated total: ", totalFrames);
    return totalFrames;
  }
  ...
  this.updateProgress = function () {
    if ( _self.progress ) {
      ...
      if (_self.getFrameNumber() >= 0) {
        ...
        var totalFrames = _self.total_no_of_frames;
        // var curFrameInSequence = _self.getCurFrameInSeq();
        var curFrameInTotal = _self.getCurFrameInTotal();
        console.log("curFrameInTotal: ", curFrameInTotal);

        var new_width = ( curFrameInTotal / ( totalFrames - 1) ) * window.innerWidth + 'px';
